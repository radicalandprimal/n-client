...
public class AutoSign extends Module {
    ...
    public AutoSign() {
        ...
        this.signTextLine2 = this.sgGeneral.add(new StringSetting.Builder()
            .name("Line 2")
        ...
        this.editSignAura = this.sgGeneral.add(new BoolSetting.Builder()
            .name("Edit Sign Aura").description("Automatically edits signs around you.").defaultValue(false)
            .build()
        );

        this.distance = this.sgGeneral.add(new DoubleSetting.Builder()
            .name("Distance").description("The distance to search for signs.")
            .defaultValue(5.0)
            .min(0.1)
            .sliderMax(10.0)
        ...
        this.delay = this.sgGeneral.add(new IntSetting.Builder()
            .name("Delay").description("The delay between editing signs.")
            .defaultValue(100)
            .min(0)
            .sliderMax(1000)
            .visible(() -> this.editSignAura.get())
    ...
    private UpdateSignC2SPacket getUpdateSignPacket(BlockPos pos, boolean front) {
        return switch (this.textPreset.get()) {
            ...
            default -> new UpdateSignC2SPacket(
                pos,
                front,
                this.replaceText(this.signTextLine1.get()),
                this.replaceText(this.signTextLine2.get()),
                this.replaceText(this.signTextLine3.get()),
                this.replaceText(this.signTextLine4.get())
            );
        };
    }
            ...
            for (int z = -1; z <= 1; z++) {
                ...
                for (Map.Entry<BlockPos, BlockEntity> entry : blockEntities.entrySet()) {
                    ...
                    if (blockEntity instanceof SignBlockEntity signBlockEntity) {
                        if (!signBlockEntity.isWaxed() && blockPos.isWithinDistance(this.mc.player.getEyePos(), this.distance.get().doubleValue()) && this.shouldUpdateText(signBlockEntity)) {
                            RaycastContext raycastContext = new RaycastContext(
                                new Vec3d(this.mc.player.getX(), this.mc.player.getEyePos().y, this.mc.player.getZ()),
                                new Vec3d((double) blockPos.getX() + 0.5, (double) blockPos.getY() + 0.5, (double) blockPos.getZ() + 0.5),
