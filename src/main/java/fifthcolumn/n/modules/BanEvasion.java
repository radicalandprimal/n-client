...
public class BanEvasion extends Module {
    ...
    public BanEvasion() {
        ...
        this.addSpacesToName = this.sgGeneral.add(new BoolSetting.Builder().name("add spaces to name").description("makes it easy to evade bans").defaultValue(false).build());
        this.evadeAndReconnect = this.sgGeneral.add(new BoolSetting.Builder().name("Toggle from evade and reconnect button").description("makes it easy to evade bans").defaultValue(true).build());
    ...
    public static boolean isSpacesToNameEnabled() {
        ...
        return banEvasion != null && banEvasion.isActive() && banEvasion.addSpacesToName.get() != false;
    }
