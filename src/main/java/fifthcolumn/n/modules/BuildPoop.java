...
public class BuildPoop extends Module {
    ...
    public BuildPoop() {
        ...
        this.placeMode = this.sgGeneral.add(new EnumSetting.Builder<PlaceMode>().name("place-mode").description("How to place").defaultValue(PlaceMode.Floor).build());
        this.range = this.sgGeneral.add(new DoubleSetting.Builder().name("range").description("Custom range to place at.").defaultValue(4.5).min(1.0).max(4.5).build());
        this.delay = this.sgGeneral.add(new IntSetting.Builder().name("delay").description("tick delay for placement. recommended 0 unless you are lagging").defaultValue(0).min(0).max(2).build());
        this.maxBlocksPerTick = this.sgGeneral.add(new IntSetting.Builder().name("max-blocks-per-tick").description("blocks to place per tick").defaultValue(1).min(1).max(2).build());
        ...
        this.rotate = this.sgGeneral.add(new BoolSetting.Builder().name("rotate").description("Spinny spinny").defaultValue(true).build());
        this.render = this.sgRender.add(new BoolSetting.Builder().name("render").description("Renders a block overlay where the blocks will be placed.").defaultValue(true).build());
        ...
        this.overlapColor = this.sgRender.add(new ColorSetting.Builder().name("overlap-color").description("The color of the lines & sides of the blocks being rendered when overlapping a block").visible(() -> this.render.get()).defaultValue(new SettingColor(0, 204, 0, 50)).build());
        ...
        this.height = this.sgStairCase.add(new IntSetting.Builder().name("height").description("Max height for each staircase placement").defaultValue(16).min(2).sliderMax(200).build());
        this.reverse = this.sgStairCase.add(new BoolSetting.Builder().name("reverse").description("start from top when staircasing").defaultValue(false).build());
        this.autoFly = this.sgStairCase.add(new BoolSetting.Builder().name("Auto Fly").description("fly while staircasing").defaultValue(false).build());
        this.stopOnIntersect = this.sgStairCase.add(new BoolSetting.Builder().name("Stop On Intersect").description("Stairs will only go up to the first intersecting block and not past it").defaultValue(false).build());
        this.snapOnIntersect = this.sgStairCase.add(new BoolSetting.Builder().name("Snap On Intersect").description("Staircase will snap to other staircases").defaultValue(false).build());
        this.snapDistance = this.sgStairCase.add(new IntSetting.Builder().name("Snap Distance").description("The distance that snapping breaks at").defaultValue(3).build());
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (this.placeMode.get() == PlaceMode.StairCase) {
            ...
            if (!this.started) {
                if (this.snapPos == null || this.stairsList.isEmpty() || !this.snapOnIntersect.get().booleanValue() || this.snapDir != this.mc.player.getHorizontalFacing() || Math.sqrt(this.mc.getCameraEntity().getBlockPos().getSquaredDistance(this.snapPos)) > (double) this.snapDistance.get().intValue()) {
                    this.snapPos = null;
                    block6: for (CardinalDirection dir : CardinalDirection.values()) {
                        ...
                        if (this.hitResult instanceof BlockHitResult && this.mc.player.getMainHandStack().getItem() instanceof BlockItem) {
                            ...
                            for (int i = 1; i <= this.height.get(); ++i) {
                                BlockState state;
                                BlockPos nextStair = new BlockPos(startBlock.getX() + x * i, this.reverse.get() != false ? startBlock.getY() - i : startBlock.getY() + i, startBlock.getZ() + z * i);
                                if (startBlock.getY() + i < 318) {
                                    this.stairsList.add(nextStair);
                                }
                                if (!this.stopOnIntersect.get().booleanValue() || (state = this.mc.world.getBlockState(nextStair)).isAir() || state.isLiquid()) continue;
        ...
        BlockIterator.register((int)Math.ceil(this.range.get()), (int)Math.ceil(this.range.get()), (blockPos, blockState) -> {
            if (Utils.squaredDistance(pX, pY, pZ, (double)blockPos.getX() + 0.5, (double)blockPos.getY() + 0.5, (double)blockPos.getZ() + 0.5) > rangeSq) {
                return;
            }
            if (this.placeMode.get() == PlaceMode.Floor) {
                ...
                if (!blockPos.equals(new BlockPos(blockPos.getX(), MathHelper.floor(pY - (double)offset), blockPos.getZ()))) {
                    return;
                }
        ...
        BlockIterator.after(() -> {
            ...
            for (BlockPos.Mutable block : this.blocksList) {
                ...
                if (this.delay1 < this.delay.get()) {
                    ++this.delay1;
                    break;
                }
                this.delay1 = 0;
                FindItemResult item = InvUtils.findInHotbar(itemStack -> this.validItem(itemStack, block));
                if (!item.found() || item.getHand() == null) return;
                if (BlockUtils.place(block, item, this.rotate.get(), 50,  this.swingHand.get(), true)) {
                    ...
                    if (this.autoFly.get().booleanValue() && this.placeMode.get() == PlaceMode.StairCase) {
                        ...
                        this.mc.player.setPosition((double) this.flyHereBlock.getX() + 0.5, (double) this.flyHereBlock.getY() + 0.5, (double) this.flyHereBlock.getZ() + 0.5);
                    }
                }
            }
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (!this.autoFly.get().booleanValue()
            || this.placeMode.get() != PlaceMode.StairCase
            || this.mc.player.getAbilities().creativeMode
            || !(event.packet instanceof PlayerMoveC2SPacket)
            || Modules.get().isActive(BetterFlight.class)
            || this.mc.player.isFallFlying()
            || this.mc.player.getVelocity().y > -1.0E-4
        ) return;
    ...
    private void onMouseButton(MouseButtonEvent event) {
        ...
        if (event.button == 1) {
            ...
            if (this.reverse.get().booleanValue()) {
    ...
    private void onRender(Render3DEvent event) {
        ...
        this.renderOverlapBlocks.forEach(renderBlock -> renderBlock.render(event, this.overlapColor.get(), this.overlapColor.get(), this.shapeMode.get()));
    }
