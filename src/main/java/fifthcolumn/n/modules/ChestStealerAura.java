...
public class ChestStealerAura extends Module {
    ...
    public ChestStealerAura() {
        ...
        this.range = this.sgGeneral.add(new DoubleSetting.Builder().name("range").description("range to steal").defaultValue(4.5).min(1.0).max(6.0).build());
        this.items = this.sgGeneral.add(new ItemListSetting.Builder().name("items").description("items to steal").build());
        this.mode = this.sgGeneral.add(new EnumSetting.Builder<Mode>().name("mode").defaultValue(Mode.Aura).build());
        this.render = this.sgRender.add(new BoolSetting.Builder().name("render").description("shows what chests have been stolen from").defaultValue(true).build());
