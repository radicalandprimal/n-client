...
public class AutoLava extends Module {
    ...
    public AutoLava() {
        ...
        this.distance = this.sgGeneral.add(new DoubleSetting.Builder().name("distance").description("how far").min(0.0).max(4.5).defaultValue(4.5).build());
        this.tickInterval = this.sgGeneral.add(new IntSetting.Builder().name("tick-interval").defaultValue(5).build());
        this.rotate = this.sgGeneral.add(new BoolSetting.Builder().name("rotate").description("rot own u").defaultValue(true).build());
        this.ticks = 0;
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        for (Entity entity : this.mc.world.getEntities()) {
            if (entity instanceof PlayerEntity && (double) this.mc.player.distanceTo(entity) <= this.distance.get() && entity != this.mc.player && !entity.isInLava()) {
                this.entity = entity;

                Vec3d go = new Vec3d(
                    entity.prevX + (entity.getX() - entity.prevX),
                    entity.prevY + (entity.getY() - entity.prevY),
                    entity.prevZ + (entity.getZ() - entity.prevZ)
                );
