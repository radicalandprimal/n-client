...
public class AutoCutie extends Module {
    ...
    public AutoCutie() {
        ...
        this.announcer = this.sgGeneral.add(new BoolSetting.Builder().name("Announcer").description("Announces new friends in chat").defaultValue(true).build());
    }
    ...
    private void addFriends(List<CopeService.Griefer> griefers) {
        ...
        griefers.stream().map(griefer -> griefer.playerName).filter(playerName -> !this.mc.player.getEntityName().equalsIgnoreCase((String)playerName)).forEach(this::addFriend);
    }

    private void addFriend(String playerName) {
        Friends friends = Friends.get();
        if (this.mc == null || this.mc.world == null || this.mc.player == null) return;
        this.mc.world.getPlayers().stream().filter(player -> playerName.equalsIgnoreCase(player.getEntityName())).findFirst().map(Friend::new).ifPresent(friend -> {
            if (friends.add(friend) && this.announcer.get().booleanValue()) {
