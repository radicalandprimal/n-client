...
public class AutoCutie extends Module {
    ...
    private void addFriends(List<CopeService.Griefer> griefers) {
        ...
        griefers.stream().map(griefer -> griefer.playerName)
            .filter(playerName -> !this.mc.player.getEntityName().equalsIgnoreCase(playerName))
            .forEach(this::addFriend);
    }
    ...
    private void addFriend(String playerName) {
        ...
        this.mc.world.getPlayers().stream()
            .filter(player -> playerName.equalsIgnoreCase(player.getEntityName()))
            .findFirst()
            .map(Friend::new)
            .ifPresent(friend -> {
