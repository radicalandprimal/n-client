...
public class AutoWither extends Module {
    ...
    public AutoWither() {
        ...
        this.range = this.sgGeneral.add(new DoubleSetting.Builder().name("range").description("Custom range to place at.").defaultValue(5.0).min(1.0).sliderMax(6.0).build());
        this.delay = this.sgGeneral.add(new IntSetting.Builder().name("delay").description("Delay").defaultValue(1).min(0).sliderMax(5).build());
        this.nametag = this.sgGeneral.add(new BoolSetting.Builder().name("nametag").description("will nametag withers if present in hotbar").defaultValue(true).build());
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (this.nametag.get().booleanValue()) {
            for (Entity entity : this.mc.world.getEntities()) {
                if (entity.getType() != EntityType.WITHER || Utils.distance(pX, pY, pZ, entity.getX(), entity.getY(), entity.getZ()) > this.range.get() || entity.hasCustomName()) continue;
                FindItemResult item = InvUtils.findInHotbar(itemStack -> this.isNameTag(itemStack));
                if (!item.found()) break;
                ...
                if (EntityUtils.getName(entity).equals(text)) continue;
        ...
        if (!this.started) {
            for (CardinalDirection dir : CardinalDirection.values()) {
                ...
                if (this.hitResult instanceof BlockHitResult && block == Blocks.SOUL_SAND) {
                    ...
                    this.sandList.add(new BlockPos(startBlock.getX() + 0, startBlock.getY() + 1, startBlock.getZ() + 0));
                    this.sandList.add(new BlockPos(startBlock.getX() + 0, startBlock.getY() + 2, startBlock.getZ() + 0));
                    ...
                    this.headList.add(new BlockPos(startBlock.getX() - ew, startBlock.getY() + 3, startBlock.getZ() - ns));
                    this.headList.add(new BlockPos(startBlock.getX() + 0, startBlock.getY() + 3, startBlock.getZ() + 0));
                    for (BlockPos pos : this.sandList) {
                        if (this.mc.world.getBlockState(pos).isReplaceable()) continue;
                        this.sandList.clear();
                        this.headList.clear();
                        return;
                    }
        ...
        BlockIterator.register((int)Math.ceil(this.range.get()), (int)Math.ceil(this.range.get()), (blockPos, blockState) -> {
            if (!this.started || Utils.squaredDistance(pX, pY, pZ, (double)blockPos.getX() + 0.5, (double)blockPos.getY() + 0.5, (double)blockPos.getZ() + 0.5) > rangeSq
                || !this.sandList.contains(blockPos) && !this.headList.contains(blockPos)
            ) {
                return;
            }
        ...
        BlockIterator.after(() -> {
            ...
            if (this.blocksList.isEmpty()) return;

            for (BlockPos.Mutable block : this.blocksList) {
                boolean isHead = this.headList.contains(block);
                FindItemResult item = InvUtils.findInHotbar(arg_0 -> this.lambda$onTickPre$5(block, isHead, arg_0));
                if (!item.found()) {
                    return;
                }
                if (item.getHand() == null) {
                ...
                this.d = this.delay.get();
                if (!BlockUtils.place(block, item, this.rotate.get(), 50, this.swingHand.get(), true)) continue;
                this.renderBlocks.add((this.renderBlockPool.get()).set(block));
                if (this.headList.size() != 1) break;
                this.headList.clear();
                break;
            }
    ...
    private void onMouseButton(MouseButtonEvent event) {
        if (!(this.mc.player.getMainHandStack().getItem() instanceof BlockItem)
            ...
            || event.button != 1
            || ((BlockItem) this.mc.player.getMainHandStack().getItem()).getBlock() != Blocks.SOUL_SAND
        ) return;
        this.started = true;
    ...
    private boolean validItem(ItemStack itemStack, BlockPos pos, boolean isHead) {
        ...
        if (isHead ? block != Blocks.WITHER_SKELETON_SKULL : block != Blocks.SOUL_SAND) {
            return false;
        }
    ...
    private void interact(Entity entity) {
        if (this.rotate.get().booleanValue()) {
            Rotations.rotate(Rotations.getYaw(entity), Rotations.getPitch(entity), -100, () -> this.mc.interactionManager.interactEntity(this.mc.player, entity, Hand.MAIN_HAND));
        } else {
            this.mc.interactionManager.interactEntity(this.mc.player, entity, Hand.MAIN_HAND);
        }
    }

    private /* synthetic */ boolean lambda$onTickPre$5(BlockPos.Mutable block, boolean isHead, ItemStack itemStack) {
        return this.validItem(itemStack, block, isHead);
    }
