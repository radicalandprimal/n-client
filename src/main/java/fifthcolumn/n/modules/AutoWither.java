...
public class AutoWither extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        BlockIterator.register((int) Math.ceil(this.range.get()), (int) Math.ceil(this.range.get()), (blockPos, blockState) -> {
            if (!this.started
                || Utils.squaredDistance(pX, pY, pZ, (double) blockPos.getX() + 0.5, (double) blockPos.getY() + 0.5, (double) blockPos.getZ() + 0.5) > rangeSq
        ...
        BlockIterator.after(() -> {
            ...
            for (BlockPos.Mutable block : this.blocksList) {
                ...
                FindItemResult item = InvUtils.findInHotbar(itemStack -> this.lambda$onTickPre$5(block, isHead, itemStack));
                if (!item.found()) {
                    return;
                }
                if (item.getHand() == null) {
