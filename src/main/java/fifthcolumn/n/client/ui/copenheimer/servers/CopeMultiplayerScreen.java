...
public class CopeMultiplayerScreen extends Screen {
    ...
    protected void init() {
        ...
        this.buttonJoin = this.addDrawableChild(ButtonWidget.builder(Text.translatable("selectServer.select"), button -> this.connect())
            .dimensions(farLeftXTopRow, buttonTopRowY, 80, 20)
            .build()
        );

        this.addDrawableChild(ButtonWidget.builder(Text.of("Update search"), button -> this.client.setScreen(new SearchParametersScreen((CopeMultiplayerScreen) this.client.currentScreen, this.copeService)))
            .dimensions(farLeftXTopRow + padding + 80, buttonTopRowY, 80, 20)
            .build()
        );

        this.addDrawableChild(ButtonWidget.builder(Text.of("Hide Server"), button -> this.serverListWidget.removeSelectedServerEntry())
            .dimensions(farLeftXTopRow + 168, buttonTopRowY, 80, 20)
            .build()
        );
        ...
        this.whiteListedButton = this.addDrawableChild(ButtonWidget.builder(Text.of("Whitelisted"), button -> {
            ...
            this.serverListWidget.removeSelectedServerEntry();
            this.updateServer(req);
        }).dimensions(farLeftXBottomRow + padding + 80, buttonBottomRowY, 80, 20).build());
        ...
        this.addDrawableChild(ButtonWidget.builder(Text.of("Proxies"), button -> this.client.setScreen(GuiThemes.get().proxiesScreen()))
            .dimensions(this.width - 75 - 3, 3, 75, 20)
            .build()
        );

        this.addDrawableChild(ButtonWidget.builder(Text.of("Accounts"), button -> this.client.setScreen(GuiThemes.get().accountsScreen()))
            .dimensions(this.width - 75 - 3 - 75 - 2, 3, 75, 20)
            .build()
        );

        this.newAltButton = ButtonWidget.builder(Text.of("New alt"), button -> {
            this.newAltButton.active = false;
            copeService.useNewAlternateAccount(session -> MeteorClient.mc.execute(() -> {
                this.newAltButton.active = true;
            }));
        }).dimensions(this.width - 75 - 3 - 150 - 2, 3, 75, 20).build();
        this.addDrawableChild(this.newAltButton);

        this.addDrawableChild(ButtonWidget.builder(Text.of("Reset account"), button -> copeService.setDefaultSession())
            .dimensions(this.width - 75 - 3 - 230 - 2, 3, 80, 20)
            .build()
        );
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (this.copeService.loading.get() && this.serverListWidget.getServers().isEmpty()) {
            context.drawCenteredTextWithShadow(this.textRenderer, this.getLoadingText(), this.width / 2, 50, 0xFFFFFF);
        }
        ...
        if (!StreamerMode.isHideAccountEnabled()) {
            ...
            context.drawTextWithShadow(this.textRenderer, Text.of("Logged in as: " + username), 10, 10, 0xA0A0A0);
        }
    }

    public void connect() {
        CopeServerListWidget.Entry entry = this.serverListWidget.getSelectedOrNull();
